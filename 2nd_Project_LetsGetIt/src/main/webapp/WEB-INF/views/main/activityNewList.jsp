...
<section>
	<div id="activityAllList">
		<div id="title">
			<span>NEW 신규 액티비티</span>
			<span id="total">${paging.total }</span>
		</div>
		<div id="list">
			<div class="activity">
				<c:forEach var="act" items="${activityNewList }">
		...
		<div id="paging">
			<c:if test="${paging.startPage>paging.blockSize }">
				<a href="${conPath }/activityNewList.do?pageNum=${paging.startPage-1}">
			...
			<c:forEach var ="i" begin="${paging.startPage }" end="${paging.endPage }">
				...
				<c:if test="${paging.currentPage!=i }">
					<a href="${conPath }/activityNewList.do?pageNum=${i}">
						<span>${i }</span>
					</a>
				</c:if>
			</c:forEach>
