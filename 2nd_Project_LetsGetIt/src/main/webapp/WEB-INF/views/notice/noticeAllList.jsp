	...
	<div id="qnaList">
		<div id="wrap">
			...
			<!--<c:if test="${not empty amdin}">
				...
				<div id="write">
					<button id="writeBtn" onclick="location.href='${conPath}/writeForm.do'">글 작성</button>
				</div>
				<div>
					<span>
			...
			<div id="paging">
				<c:if test="${paging.startPage>paging.blockSize }">
					<a href="${conPath }/noticeAllList.do?pageNum=${paging.startPage-1}">
				...
				<c:forEach var ="i" begin="${paging.startPage }" end="${paging.endPage }">
					...
					<c:if test="${paging.currentPage!=i }">
						<a href="${conPath }/noticeAllList.do?pageNum=${i}">
							<span>${i }</span>
						</a>
					</c:if>
				</c:forEach>
				<c:if test="${paging.endPage<paging.pageCnt }">
					<a href="${conPath }/noticeAllList.do?pageNum=${paging.endPage+1}">
						<span class="next">〉</span>			
					</a>
				</c:if>
			</div>
