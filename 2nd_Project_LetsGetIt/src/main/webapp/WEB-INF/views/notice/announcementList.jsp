		...
		<div id="wrap">
			...
			<c:if test="${not empty amdin}">
				...
				<div>
					...
					<span>
						<a href="${conPath }/announceList.do">발표</a>
					</span>
				</div>
				<div id="list">
			...
			<div id="paging">
				<c:if test="${paging.startPage>paging.blockSize }">
					<a href="${conPath }/announcementList.do?pageNum=${paging.startPage-1}">
				...
				<c:forEach var ="i" begin="${paging.startPage }" end="${paging.endPage }">
					...
					<c:if test="${paging.currentPage!=i }">
						<a href="${conPath }/announcementList.do?pageNum=${i}">
							<span>${i }</span>
						</a>
					</c:if>
				</c:forEach>
				<c:if test="${paging.endPage<paging.pageCnt }">
					<a href="${conPath }/announcementList.do?pageNum=${paging.endPage+1}">
						<span class="next">〉</span>			
					</a>
				</c:if>
			</div>
