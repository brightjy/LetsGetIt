...
<div id="activityAllList">
	<div id="title">
			<span>내가 좋아한 액티비티</span>
			<span id="total">${paging.total }</span>
	</div>
	<div id="list">
		<div class="activity">
			<c:forEach var="act" items="${memberLikeActivityList }">
		...
		<div id="paging">
			<c:if test="${paging.startPage>paging.blockSize }">
				<a href="${conPath }/myActivityLikeList.do?actId=${actId }&pageNum=${paging.startPage-1}">
			...
			<c:forEach var ="i" begin="${paging.startPage }" end="${paging.endPage }">
				...
				<c:if test="${paging.currentPage!=i }">
					<a href="${conPath }/myActivityLikeList.do?actId=${actId }&pageNum=${i}">
						<span>${i }</span>
					</a>
				</c:if>
			</c:forEach>
			<c:if test="${paging.endPage<paging.pageCnt }">
				<a href="${conPath }/myActivityLikeList.do?actId=${actId }&pageNum=${paging.endPage+1}">
					<span class="next">〉</span>			
				</a>
			</c:if>
		</div>
